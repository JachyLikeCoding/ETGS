...
from cryoET_3dgs.scene.dataset_readers import readWbpInfo
from cryoET_3dgs.arguments import ModelParams, DatasetParams
from cryoET_3dgs.utils.system_utils import searchForMaxIteration
from cryoET_3dgs.utils.tilt_utils import tiltList_from_tiltInfos, tilt_to_JSON
...
class Scene:
    ...
    def __init__(self, args: ModelParams, data: DatasetParams, 
                 gaussians: GaussianModel_cryoET, load_iteration=None, shuffle=True, 
        ...
        print(f'[debug] scene model path --- {self.model_path}')
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = readWbpInfo(args.source_path, args.images, args.eval, data, args.white_background, args.if_align, if_render)
        else:
        ...
        self.bbox = torch.tensor([[-data.volume_x/2, -data.volume_y/2, -data.volume_z/2],
                              [data.volume_x/2, data.volume_y/2, data.volume_z/2]], device="cuda")
        print('[debug] bbox:', self.bbox)
        
        # 加载或创建高斯模型
        if self.loaded_iter:
            print('[debug] load gaussian model.....')
            self.gaussians.load_ply(os.path.join(self.model_path, 
                                                 "gaussians", 
                                                 "iteration_" + str(self.loaded_iter), 
                                                 "gaussian_points.ply"))
        elif data.random_init == True:
            self.gaussians.create_random_gaussians(data.intensity_threshold, spatial_lr_scale=1, image_width=data.image_width, image_height=data.image_height, 
                                grid_size=data.grid_size, volume_x=data.volume_x, volume_y=data.volume_y, volume_z=data.volume_z, factor=data.scale_factor, scale_init=2.0)
        else: 
            # 根据场景信息中的点云数据创建高斯模型
            # self.gaussians.create_from_wbp(scene_info.point_cloud, intensity_threshold=data.intensity_threshold, 
            #                                init_size=10000, spatial_lr_scale=1, image_width=data.image_width, image_height=data.image_height, 
            #                                grid_size=data.grid_size, volume_x=data.volume_x, volume_y=data.volume_y, volume_z=data.volume_z, factor=data.scale_factor)
            self.gaussians.create_hierarchical_gaussian(scene_info.point_cloud, intensity_threshold=data.intensity_threshold, 
                                           init_size=100000, spatial_lr_scale=1, image_width=data.image_width, image_height=data.image_height, 
                                           grid_size=data.grid_size, volume_x=data.volume_x, volume_y=data.volume_y, volume_z=data.volume_z, factor=data.scale_factor, if_whitebg=args.white_background)
        ...
        if points.shape[0] < 1000000:
            self.gaussians.visualize_gaussians(points, intensities, scales, data.volume_x, data.volume_y, data.volume_z)


    def save(self, iteration, data: DatasetParams):
        """
        保存当前迭代下的3D高斯模型点云
        """
        gaussian_points_path = os.path.join(self.model_path, "gaussians/iteration_{}".format(iteration))
        print(f'[debug] save gaussian points: {gaussian_points_path}')
        ...
        self.gaussians.save_gaussianpoints_rgb2(os.path.join(gaussian_points_path, "gaussian_points_rgb2.ply"))
