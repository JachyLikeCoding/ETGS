	...
	const bool debug)
{
	...
	// printf("\n.... VoxelizeGaussiansCUDA finish rendered = %d", rendered);
	...
	const bool debug)
{
	...
	torch::Tensor dL_dintensity = torch::zeros({P, 1}, means3D.options());
	...
	return std::make_tuple(dL_dintensity, dL_dmeans3D, dL_dcov3D, dL_dscales, dL_drotations);
}