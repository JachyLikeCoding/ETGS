...
__device__ float4 computeCov2D(const float3& mean, float kernel_size, const float* cov3D, const float* viewmatrix)
{
	...
	if (cov3D[0] > 1e6 || cov3D[1] > 1e6 || cov3D[2] > 1e6 || cov3D[3] > 1e6 || cov3D[4] > 1e6 || cov3D[5] > 1e6) {
		printf("Abnormal cov3D values: %f, %f, %f, %f, %f, %f\n", cov3D[0], cov3D[1], cov3D[2], cov3D[3], cov3D[4], cov3D[5]);
	}
	...
	const float det_0 = max(1e-6, hata * hatd - hatb * hatb);
	const float det_1 = max(1e-6, (hata + kernel_size) * (hatd + kernel_size) - hatb * hatb);
	
	float coef = sqrt(det_0 / (det_1 + 1e-6));
	// 检查数值合法性
	if (det_0 <= 0.0f || det_1 <= 0.0f || coef > 1.0f || isnan(coef)) {
		printf("Warning: Invalid cov determinant! det0=%.8f, det1=%.8f, coef=%.8f\n", det_0, det_1, coef);
		coef = 0.0f;
	}
	...
	//! Compute integration bias factor mu
	...
	bool prefiltered) 			//是否对输入进行了预过滤
{
	...
	float3 max_bound = make_float3(X, Y, Z);     // 根据实际数据设置(simulate.yaml)
    ...
    // 从NDC坐标系转到像素坐标系
	float2 point_image = {(p_proj.x + X/2)/X * W, (p_proj.y + Y/2)/Y * H}; 
	...
	float* __restrict__ out_intensity) 			//最终渲染结果的数组
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)  // 使用rounds控制循环的迭代次数，每次迭代处理一批点云数据。
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) //在当前批次的循环中，每个线程处理一条点云数据。
		{
			...
			const float intensity_contrib = conic_intensity.w * mu * exp(power); // 这里计算的是当前点的密度（不能直接算成高斯中心的值）
	...
	if (inside)
	{
		...
		out_intensity[pix_id] = max(intensity_accum, 0.0f) + bg_color[0];
	}
}
